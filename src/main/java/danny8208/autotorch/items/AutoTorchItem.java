    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (!worldIn.isRemote && EnabledUtil.isEnabled(stack)) {
            if (!entityIn.isSpectator() &&
                    worldIn.isAirBlock(new BlockPos(entityIn.getPosX(), entityIn.getPosY(), entityIn.getPosZ())) &&
                    Block.hasSolidSideOnTop(worldIn, new BlockPos(entityIn.getPosX(), entityIn.getPosY(), entityIn.getPosZ()).down()) &&
                    TorchUtil.getTorches(stack) > 0 &&
                    worldIn.getLight(new BlockPos(entityIn.getPosX(), entityIn.getPosY(), entityIn.getPosZ())) <= 7) {
                worldIn.setBlockState(new BlockPos(entityIn.getPosX(), entityIn.getPosY(), entityIn.getPosZ()), Blocks.TORCH.getDefaultState());
